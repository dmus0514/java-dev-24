...
public class MessageController {
    ...
    public static final String ROOM_1408_STR = "1408";
    public static final Long ROOM_1408_ID = 1408L;
    ...
    public void getMessage(@DestinationVariable("roomId") String roomId, Message message) {
        if (roomId.equals(ROOM_1408_STR)) {
            logger.warn("Received message is for 1408 room which is not allowed");
            return;
        }
        ...
        template.convertAndSend(
                String.format("%s%s", TOPIC_TEMPLATE, ROOM_1408_STR), new Message(HtmlUtils.htmlEscape(message.messageStr())));
    }
    ...
    public void handleSessionSubscribeEvent(SessionSubscribeEvent event) {
        ...
        logger.info("handleSessionSubscribeEvent > subscription for:{}", simpDestination);
    ...
    private Flux<Message> getMessagesByRoomId(long roomId) {
        String uri = (roomId == ROOM_1408_ID) ? "/msg/all" : String.format("/msg/%s", roomId);
